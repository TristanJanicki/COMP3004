...
def getCurrencyData(fromCurr, toCurr, api_key="", return_csv_obj=False, skip_existing=False):
    ...
    path = "../stock_data/" + fromCurr + "_" + toCurr + "Data.csv"
    if os._exists(path):
        return path
    path, exists = genCSV(ticker=fromCurr + "_" + toCurr, function="FX", apiUrl=url, skip_existing=skip_existing)
    print("PATH: ", path)
    if return_csv_obj == True:
        return csv.reader(open(path)), exists
    return path, exists
